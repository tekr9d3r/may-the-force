...
type SnapResult = Awaited<ReturnType<SnapFunction>>;
...
const snap: SnapFunction = async (ctx) => {
  ...
  const totalVotes = lightVotes + darkVotes;

  if (ctx.action.type !== "post" || !side) {
    return landingScreen(base, lightForce, darkForce, totalVotes);
  }

  const fid = ctx.action.user?.fid;
  if (!fid) return landingScreen(base, lightForce, darkForce, totalVotes);
  ...
  if (existing) {
    ...
    return alreadyVotedScreen(base, existing, lightForce, darkForce, lightPct);
  }
  ...
  return votedScreen(base, side, forceScore, newLightForce, newDarkForce, newLightPct);
};
    ...
    ui: {
      ...
      elements: {
        page: {
          type: "stack",
          props: {},
          children: ["title", "subtitle", "chart", "btnLight", "btnDark", "footer"],
        },
        title: {
          type: "text",
          props: { content: "May the 4th Be With You ⚡", weight: "bold", align: "center" },
        },
        subtitle: {
          type: "text",
          props: {
            content: "Lower FID + more followers = more Force. Which side calls to you?",
            size: "sm",
            align: "center",
          },
        },
        ...
        btnLight: {
          type: "button",
          props: { label: "Join the Light Side ☀️", variant: "primary" },
          on: { press: { action: "submit", params: { target: `${base}/?side=light` } } },
        },
        btnDark: {
          type: "button",
          props: { label: "Join the Dark Side 🌑", variant: "secondary" },
          on: { press: { action: "submit", params: { target: `${base}/?side=dark` } } },
        },
        footer: {
          type: "text",
          props: { content: footer, size: "sm", align: "center" },
        },
      },
    },
  };
...
function votedScreen(
  ...
  return {
    ...
    ...(isLight ? { effects: ["confetti"] as ["confetti"] } : {}),
    ui: {
      root: "page",
      elements: {
        page: {
          type: "stack",
          props: {},
          children: ["title", "forceLabel", "chart", "pctCaption", "shareBtn"],
        },
        title: {
          type: "text",
          props: { content: `You joined the ${sideLabel}`, weight: "bold", align: "center" },
        },
        forceLabel: {
          type: "text",
          props: {
            content: `Your Force Power: ${formatForce(forceScore)} ⚡ — you shifted the balance!`,
            size: "sm",
            align: "center",
          },
        },
        ...
        pctCaption: {
          type: "text",
          props: {
            content: `${isLight ? "Light" : "Dark"} Side holds ${leadingPct}% of the Force`,
            weight: "bold",
            align: "center",
          },
        },
        shareBtn: {
          type: "button",
          props: { label: "Share Your Allegiance", variant: "primary", icon: "share" },
          on: {
            press: {
              action: "compose_cast",
    ...
    ui: {
      ...
      elements: {
        ...
        title: {
          type: "text",
          props: {
            content: `You already chose the ${sideLabel}`,
            weight: "bold",
            align: "center",
          },
        },
        forceLabel: {
          type: "text",
          props: {
            content: `Your Force Power: ${formatForce(vote.force)} ⚡`,
            size: "sm",
            align: "center",
          },
        },
